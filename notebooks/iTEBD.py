...
def Calc_Energy_infinite(Env_left,Env_right,Tn,lam,Jz,Jxy,hx,hz,D):
    period = len(Tn)
    m = Tn[0].shape[0]
    ...
    for i in range(period):
        ...
        mz[i] = np.real(TEBD.Contract_one_site(Env_left[i],Env_right[i],Tn[i],lam[i],Sz)/norm)
    ...
    E = (Jz * np.sum(zz) + 0.5 * Jxy * (np.sum(pm) + np.sum(mp)) -hx * np.sum(mx) -hz * np.sum(mz) + D * np.sum(z2)) / period
    return E

def iTEBD_IT_Simulation(m,Jz,Jxy,hx,hz, D,chi_max,tau_max,tau_min,tau_step,period = 2, inv_precision=1e-10,second_ST=False,tensor_dtype=np.dtype(float),output_dyn=False,output_dyn_num=100,output=False):
    tau_factor = (tau_min/tau_max)**(1.0/tau_step)
    output_step = tau_step/output_dyn_num
    
    Ham = TEBD.set_Hamiltonian_S(m,Jz,Jxy,hx,hz,D,position=0)
    ...
    for n in range(tau_step):
        ...
        if (output_dyn and n%output_step == 0):
            ...
            E = Calc_Energy_infinite(Env_left,Env_right,Tn,lam,Jz,Jxy,hx,hz,D)
            prob = lam[period//2]**2
            S = -np.sum(prob * np.log(prob)) # Entropy
            print(f"##Dyn {T} {E} {np.sum(mz)/period} {S} {mz}")
            T_list.append(T)
            E_list.append(E)
            mz_list.append(mz)
            S_list.append(S)
    ...
    if output:
        ...
        print(m, Jz, Jxy, hx, hz, D, E, np.sum(mz)/period, S)
...
def iTEBD_RT_Simulation(m,Jz,Jxy,hx,hz,D,chi_max,dt,t_step,init_Tn, init_lam, period = 2, inv_precision=1e-10,second_ST=False,tensor_dtype=np.dtype(complex),output_dyn=False,output_dyn_num=100,output=False):
    ...
    expH = linalg.expm(-1.0j*dt*Ham).reshape(m,m,m,m)
    if (second_ST):
        expH2 = linalg.expm(-0.5j*dt*Ham).reshape(m,m,m,m)
...
def parse_args():
    ...
    parser.add_argument('-Jz', metavar='Jz',dest='Jz', type=float, default=-1.0,
                        help='anisotoropy for SzSz interaction (default = -1.0)')
    parser.add_argument('-Jxy', metavar='Jxy',dest='Jxy', type=float, default=0.0,
                        help='anisotoropy for SxSx+SySy interaction (default = 0.0)')
    ...
    parser.add_argument('-period', metavar='period',dest='period', type=int, default=2,
                        help='period of unit cell')
...
if __name__ == "__main__":
    ...
    if args.output_dyn:
        Tn, lam,T_list,E_list,mz_list = iTEBD_IT_Simulation(args.m,args.Jz,args.Jxy,args.hx,args.hz,args.D,args.chi_max,args.tau_max,args.tau_min,args.tau_step,args.period,args.inv_precision,args.second_ST,tensor_dtype,args.output_dyn,args.output_dyn_num,output=True)
    else:
        Tn, lam = iTEBD_IT_Simulation(args.m,args.Jz,args.Jxy,args.hx,args.hz,args.D,args.chi_max,args.tau_max,args.tau_min,args.tau_step,args.period,args.inv_precision,args.second_ST,tensor_dtype,args.output_dyn,args.output_dyn_num,output=True)
